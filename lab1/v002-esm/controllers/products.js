...
function updateProduct(req, res) {
  ...
  const index = products.findIndex((element) => element.id == id); // :fix001
...
function getProduct(req, res) {
  if (req.url === "/api/products") {
    ...
    const product = products.find((element) => element.id == id); // :fix001
    if (product) {
      res.writeHead(200, { "content-type": "application/json" });
...
function createProduct(req, res) {
  ...
  req.on("end", () => {
    ...
    try {
      ...
      products.push(newProduct); // :fix001
...
function deleteProduct(req, res) {
  ...
  const deletedProduct = products.splice(index, 1)[0]; // :fix001
  res.writeHead(200, { "content-type": "application/json" }); // :fix001
  return res.end(
